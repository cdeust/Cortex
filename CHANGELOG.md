...
### Fixed
- **`check_doc_claims.py`'s 25 surviving mutants, closed, plus 9 more the same rescope surfaced** (#235). A scoped mutmut run over the doc-claim gate left 25 survivors in its canonical-source helpers (`canonical_reference_count` 10, `canonical_tool_counts` 7, `canonical_version` 3, `canonical_mechanism_count` 3, `read` 2) — all missing-branch and error-message assertions no test exercised (e.g. `canonical_version()`'s "not found" arm had no test at all; `canonical_reference_count`'s `split(..., 1)` vs an unlimited/`rsplit` mutant was indistinguishable with only one `"## References"` heading in every fixture). Fixing `also_copy` to include `assets/` (missing from the committed mutation config, so the baseline run could not even complete — `RepositoryTests` needs the real badge SVGs) surfaced 9 more survivors outside the issue's own table, in `check_badge`, `check_no_hotlinked_badges`, `check_no_conflict_markers`, and `check_scanned_json_parses` — message-content mutants and two `continue`→`break` mutants that would silently abort scanning the rest of `SCANNED_FILES` after the first missing/skipped file. `main()` itself had zero mutation coverage (37 "no tests" mutants, mutmut's coverage-gap category) since every existing test called `collect_failures` directly rather than the CLI entry point; a new `MainTests` class exercises every arm (ClaimError abort → exit 2, failures report → exit 1, success + exemption count → exit 0, `--test-count` forwarding) by capturing stdout/stderr, plus the parser's declared `description`/`help` text via method-level spies (not a subclass — argparse's own `ArgumentParser.__init__` does `super(ArgumentParser, self)`, an explicit not zero-arg `super()`, so replacing the `argparse.ArgumentParser` module attribute makes that lookup recurse forever). Two mutants are documented equivalents rather than killed: `encoding="UTF-8"` is the same codec as `"utf-8"` under CPython's case-insensitive codec lookup, and dropping an explicit `default=None` from an argparse optional argument changes nothing since that is argparse's own implicit default. Final: 325 of 327 mutants killed, 2 documented equivalent, 0 unexplained survivors. Suite grows 6673 → 6692 (19 new tests; rebased onto #263's concurrent 6609 → 6673 growth).

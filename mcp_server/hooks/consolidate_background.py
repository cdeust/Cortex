...
def _load_handler():  # noqa: ANN201 -- returns the lazily-imported async handler callable
    """Import the consolidate handler; exit 1 on failure without crashing
    the spawn loop that launched this process."""
    try:
...
def _run_cycle(handler, args: dict[str, Any]) -> dict[str, Any]:  # noqa: ANN001 -- see _load_handler
    """Run one consolidate cycle; exit 1 on a raised failure (non-fatal:
    the stamp stays untouched so the next session retries)."""
    start = datetime.now(timezone.utc).isoformat(timespec="seconds")
    print(f"[bg-consolidate] starting at {start} deep={args['deep']}", file=sys.stderr)
    try:
...
def main() -> None:
    ...
    result = _run_cycle(handler, _build_args(deep))

    status = result.get("status", "unknown")
    print(
        f"[bg-consolidate] finished status={status} "
    ...
    # failure raises and we exit non-zero inside _run_cycle above.
    try:
        _write_stamp()
    except OSError as exc:
        print(f"[bg-consolidate] stamp write failed: {exc}", file=sys.stderr)

    _log_wiki_summary(result)

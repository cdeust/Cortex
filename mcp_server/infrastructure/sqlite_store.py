...
def _fts_augment(content: str) -> str:
    """Append identifier sub-tokens to FTS content.
...
def _register_json_codec() -> None:
    """Register the SQLite JSON declared-type converter. Other column types are
    unaffected.
...
class SqliteMemoryStore(
    ...
    def _migrate_fts_code_tokenize(self) -> None:
        ...
        table and reindex every memory with code-aware sub-tokens.
    ...
    def _migrate_heat_to_heat_base(self) -> None:
        """Phase 3 A3 migration (SQLite parity): rename heat → heat_base, add
        heat_base_set_at + no_decay.

    ...
    "inputSchema": {
        ...
        "properties": {
            ...
            "origin_tool": {
                ...
                "description": (
                    # source: ADR-0442
                    "Name of the tool producing content on automatic capture paths. "
                    "Capture origin is determined from this channel, not from content "
                    "text. Network-tool output cannot use content-derived write-gate "
                    "bypasses. Distinct from 'source' (the writing pipeline) and the "
                    "provenance grade. Omit for deliberate user writes."
                ),
            ...
            "write_class": {
                ...
                "description": (
                    # source: ADR-0442
                    "Explicit write class — the caller states what kind of write this "
                    "is; no source-string inference happens when this is set. One of: "
                    "'auto' (unattended tool-output capture — subject to the standard "
                    "novelty gate AND homeostatic heat regulation; this is the ONLY "
                    "class ever folded/re-suppressed toward the domain heat target); "
                    "'deliberate' (a considered, user- or agent-authored fact, "
                    "decision, or lesson — NEVER rejected by the gate for low novelty "
                    "and NEVER heat-folded; near-duplicates are still "
                    "merged/linked/superseded by curation); 'derived' "
                    "(machine-synthesized from an existing corpus — "
                    "consolidation/memify, CLS semantic promotion, sleep-compute "
                    "auto-narration — judged by idempotence markers instead of the "
                    "novelty gate); 'mechanical' (one-shot bulk import or structural "
                    "indexing — backfill, ingest_*, seed_*, codebase_analyze, wiki "
                    "pointer sync — bypasses the gate entirely, force=true "
                    "semantics). Omit to default to 'deliberate' via source-based "
                    "fallback classification. An unrecognized value is rejected with "
                    "a ValidationError, never silently reinterpreted."
                ),
                "enum": ["auto", "deliberate", "derived", "mechanical"],
            ...
            "initial_heat": {
                ...
                "description": (
                    # source: ADR-0442
                    "Initial heat override [0.0, 1.0] for backfill and import paths. "
                    "Defaults to 1.0 for live writes. Surprise boost still applies on "
                    "top."
                ),
                "minimum": 0.0,
                "maximum": 1.0,

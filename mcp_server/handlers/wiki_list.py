...
schema = {
    ...
    "description": (
        # source: ADR-0461
        "Enumerate every authored wiki page under "
        "~/.claude/methodology/wiki/, filesystem-walked from the wiki "
        "root. Optionally restrict by kind (adr, specs, guides, "
        "reference, conventions, lessons, notes, journal, files). Two "
        "filters are applied by default and can be opted out of: (1) "
        "redirect stubs (frontmatter ``redirect_to:`` or "
        "``redirect_id:``) are excluded — pass ``include_redirects: "
        "true`` to see them; (2) auto-generated pages (frontmatter "
        "``provenance: auto-generated``, produced by "
        "``codebase_analyze``) are excluded — pass "
        "``include_auto_generated: true`` to see them. Read-only; never "
        "modifies anything. Distinct from `wiki_reindex` which generates "
        "the .generated/INDEX.md from the same enumeration, and from "
        "`wiki_read` which fetches one page's content. Latency <200ms on "
        "a 9000-page wiki. Returns {root, count, pages, redirect_count, "

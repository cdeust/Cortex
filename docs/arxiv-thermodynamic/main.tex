...
benchmarks, Cortex reaches LongMemEval R@10\,$=$\,97.8\%
(vs.\ 78.4\% paper-best), LoCoMo R@10\,$=$\,92.6\%, and BEAM
Overall\,$=$\,0.591 (vs.\ 0.329 paper-best).  We characterise when
...
R@10\,$=$\,97.8\% versus 78.4\% paper-best; on BEAM
\citep{Tavakoli2026}, 0.591 Overall versus 0.329 paper-best; on
LoCoMo \citep{Maharana2024}, R@10\,$=$\,92.6\%.  All numbers are
...
\begin{enumerate}
  ...
  \item Empirical results on three long-term-memory benchmarks
        (\S\ref{sec:empirical}): R@10\,$=$\,97.8\% on LongMemEval
        ($+19.4$\,pp over paper-best), R@10\,$=$\,92.6\% on LoCoMo,
        and 0.591 Overall on BEAM ($+79.6\%$ relative).
\end{enumerate}
...
BEAM        & ICLR 2026 & Overall & \textbf{0.591}  & 0.329  \\
\bottomrule
\end{tabular}
\end{table}

The $+19.4$\,pp absolute gain on LongMemEval R@10 and the $+79.6\%$
...
RECONSOLIDATION                 & 0.8202 & 0.931 & $+$0.0076  & $+$0.011   & NO   \\
CO\_ACTIVATION                  & 0.8268 & 0.940 & $+$0.0010  & $+$0.001   & NO   \\
ADAPTIVE\_DECAY                 & 0.8441 & 0.962 & $-$0.0163  & $-$0.020   & NO   \\
BASELINE\_WITH\_CONSOLIDATION   & 0.8264 & 0.940 & 0          & 0          & self \\
CASCADE                         & 0.8272 & 0.941 & $-$0.0008  & $-$0.001   & WITH \\
INTERFERENCE                    & 0.8260 & 0.939 & $+$0.0004  & $+$0.001   & WITH \\
HOMEOSTATIC\_PLASTICITY         & 0.8289 & 0.945 & $-$0.0025  & $-$0.005   & WITH \\
SYNAPTIC\_PLASTICITY            & 0.8264 & 0.940 & $\phantom{-}0.0000$ & 0 & WITH \\
MICROGLIAL\_PRUNING             & 0.8253 & 0.939 & $+$0.0011  & $+$0.001   & WITH \\
TWO\_STAGE\_MODEL               & 0.8276 & 0.941 & $-$0.0012  & $-$0.001   & WITH \\
EMOTIONAL\_DECAY                & 0.8249 & 0.940 & $+$0.0015  & $\phantom{-}0.000$ & WITH \\
TRIPARTITE\_SYNAPSE             & 0.8268 & 0.941 & $-$0.0004  & $-$0.001   & WITH \\
SCHEMA\_ENGINE                  & 0.8268 & 0.941 & $-$0.0004  & $-$0.001   & WITH \\
\bottomrule
\end{tabular}
\end{center}
...
bug, and constitute the empirical resolution finding above.  A
re-run of the consolidation-only group on post-fix bytes is
documented as a follow-up; the architectural-mismatch resolution
...
\subsubsection{Verification surfaced a second production fix: plasticity result-shape contract}
...
A re-run of the \texttt{BASELINE\_WITH} and consolidation-only rows
on post-\texttt{5f737fe} bytes is the natural follow-up.  The
\S\ref{sec:per-mechanism} narrative does not change as a result; the
consolidation-only deltas are already at the noise floor in the
pre-fix bytes, so the most likely outcome of the re-run is tighter
confirmation of the same structure.  We declare this here rather
than amend silently because the verification campaign's evidence is
the load-bearing argument of \S\ref{sec:per-mechanism}, and the
integrity of that argument requires disclosing every code-path
artefact that touched the numbers.
...
\begin{itemize}
\item \emph{Single-seed on both benchmarks.}  Each row in
  \S\ref{sec:per-mech-table} (17 rows, LME-S, $n = 500$) and
  \S\ref{sec:locomo-evidence} (14 rows, LoCoMo, $n = 1986$) is run
  once.  Per-question noise averages down by $\sqrt{n}$; empirical
  per-row noise floor is $\approx \pm 0.001$ MRR on LME-S and
  $\approx \pm 0.002$ MRR on LoCoMo.  $\Delta$MRR magnitudes below
  the relevant threshold are not interpretable as causal
  contributions.  The paper-bearing claims of \S\ref{sec:per-mechanism}
  are the \emph{category-specialization pattern}
...
The 97.8\% / 92.6\% / 0.591 numbers are corroborations of the

...
All 47 empty `directory_context` rows also have an empty `domain`. None can
be assigned by the plan's domain mapping rule. In particular, memory 4353879
cannot satisfy the acceptance test until the owner explicitly assigns it
to japonais-2027. Without that assignment, the restored-copy `auto_recall`
still includes its Score audit decision in anthropic-partnership. This is
an observed failed acceptance condition, not a completed production fix.
The parent Team Decisions query still returns unresolved row 4356519; its
project also requires owner review. The full SessionStart launcher run
timed out after 60 seconds during validation; direct query evidence and
subprocess fixture tests do not replace final installed-plugin acceptance.

Unresolved IDs, left global without changing their project:

4254394, 4342464, 4343116, 4347543, 4349020, 4349043, 4349560, 4349960, 4349976, 4350007, 4351552, 4351562, 4351771, 4352557, 4352618, 4353539, 4353601, 4353612, 4353879, 4354421, 4354708, 4355514, 4355539, 4355775, 4356057, 4356262, 4356365, 4356519, 4356548, 4356730, 4357099, 4359407, 4359553, 4359753, 4359754, 4359755, 4359773, 4360000, 4361139, 4362817, 4365424, 4365650, 4367130, 4367241, 4367268, 4367557, 4367684
...
1. Review ADR-1083 and the unresolved ID list. Create a private mapping file

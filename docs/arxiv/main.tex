...
\title{Priority-Budgeted Stage-Aware Context Assembly\\for Long-Context Memory Retrieval}
...
for global orientation.  Our three-phase structure (current stage,
...
management, and agent architectures and found no published precedent
...
The MMR objective with $\lambda < 1$ is submodular (the
max-similarity penalty is submodular as the pointwise maximum of
linear functions), so this guarantee applies.
...
\begin{enumerate}
  ...
  \item \textbf{Assembly.}  The three phases produce a structured

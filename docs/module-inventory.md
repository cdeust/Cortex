...
- `wiki_sections.py` — One reading of a draft's sections for every consumer (`is_text`, `heading_of`, `body_of`, `titled_sections`): the Markdown renderer, the `wiki.pages` mirror and the curator all read a section through it, so a file and its row describe the same page (ADR-1071)
...
57 standalone tools + 3 upstream-integration tools conditionally registered
(60 total) + a `handlers/consolidation/` subpackage + per-tool helpers. See

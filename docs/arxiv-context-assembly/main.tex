...
LongMemEval, 94.35\% R@10 on LoCoMo (E1 v3 post-fix run, May 2026;
code SHA \texttt{2f45bcb}).  The five-signal fusion
...
\subsection{Active Retrieval (Considered, Not Adopted)}
\label{sec:active-retrieval}

Following MIRIX \citep{Wang2025}, we prototyped an optional active
retrieval layer intended to reformulate the query before it enters
the WRRF pipeline, with two candidate implementations:
...
(\texttt{rules/coding-standards.md} \S9, ``if it's built, it must be
called''), the unwired prototype module (\texttt{active\_retrieval.py})
was removed from the codebase on 2026-07-30 (issue \#201) rather than
left present but uncalled. Ablation against query reformulation, if
pursued, would require a fresh implementation wired at the pipeline's
query-intake step --- the design sketched above remains a reasonable
starting point, but it is no longer code that exists in this
repository.
...
strong baseline: 98.4\% R@10 on LongMemEval, 94.35\% R@10 on LoCoMo
(E1 v3 post-fix run, May 2026; code SHA \texttt{2f45bcb}), and 0.591
MRR on BEAM-100K.
...
\bottomrule
\multicolumn{4}{l}{\footnotesize This table predates the issue \#196 \texttt{stage\_phases.py}} \\
\multicolumn{4}{l}{\footnotesize addition and the issue \#228 \texttt{condensers.py} split;} \\
\multicolumn{4}{l}{\footnotesize \texttt{active\_retrieval.py} (was 189 lines, Wang \& Chen} \\
\multicolumn{4}{l}{\footnotesize backing) is removed as of issue \#201 --- see} \\
\multicolumn{4}{l}{\footnotesize \texttt{docs/module-inventory.md} for the current file count.} \\
\end{tabular}
\end{table}

...
# source: scripts/docker_smoke_client.py's own request batch — ties this
# reproduction to docker_smoke.sh's LITERAL historical failure message
# ("no valid tools/list response (id=3)"), not just the same defect class
...
class TestDockerSmokeToolsListLostBeforeDrain:
    """Pins docker_smoke.sh's LITERAL historical failure ('no valid
    tools/list response (id=3)', empty stderr, no JSON-RPC error frame) to
    this mechanism, using the script's own request id and method — not just
    the same defect class under `tools/call`/id=2 above. Before
    `scripts/docker_smoke_client.py` existed, `docker_smoke.sh` drove the
    container with exactly the shape the first test below reproduces:
    `printf | docker run` writes the whole batch and closes the container's
    stdin the instant the write finishes, without ever reading a response
    first.
    """

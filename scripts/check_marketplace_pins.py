...
manifests sat a three-way split with its tag (AP #67). Failure classes:
  ...
  PIN_BEHIND_RELEASE    github-source pin < that repo's latest release, but a
                        release matching the pin DOES exist.
  PIN_VERSION_UNPUBLISHED  the pinned version has no matching tag/release in
                        the target repo AT ALL — not "behind", genuinely
                        never cut. `PIN_BEHIND_*` only ever compared the pin
                        to the *latest* known tag, so a pin sitting AHEAD of
                        every real release (a version nobody ever tagged)
                        read as "current" and passed silently. Incident:
                        Cortex's own marketplace.json pinned
                        `hypermnesia-mcp-viz` at cortex-viz version "3.0.0"
                        for six days; cortex-viz's own tag history never
                        contained a v3.0.0 — v2.8.0 was, and remained, the
                        latest real tag. `3.0.0 < 2.8.0` is false, so the old
                        `PIN_BEHIND_RELEASE` check never fired: a pin ahead
                        of the truth is invisible to a "behind" comparison.
                        Diagnostic method: put the declared versions and the
                        actually-tagged versions side by side; the gap is
                        only visible in the join, never in either list read
                        alone, and never in the manifest file by itself — the
  ...
  REGISTRY_VERSION_STALE  the public MCP registry (registry.modelcontextprotocol.io)
                       serves a version != this repo's authoritative one for
                       server.json's own `name`. A THIRD version surface,
                       independent of the marketplace pin and PyPI: measured
                       2026-08-10, io.github.cdeust/hypermnesia-mcp was
                       published at 4.17.1 while the tag/server.json/PyPI
                       were already at 4.17.2 — the registry publish step
                       lived only in prose (no committed workflow ran it),
                       same failure shape as #179 and PIN_VERSION_UNPUBLISHED
                       above. Equality, not "behind": a registry entry ahead
                       of the repo (a republish of an untagged version) is
                       exactly as false as one that lags.
